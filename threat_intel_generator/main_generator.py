...
class ThreatIntelGenerator:
    ...
    def load_nvd_data(self, cve_id: str) -> Optional[Dict[str, Any]]:
        """加载 NVD 原始 JSON 数据"""
    ...
    def load_web_data(self, cve_id: str) -> Optional[str]:
        """加载 Web 搜索摘要文本"""
    ...
    def create_intel_prompt(self, nvd_data: Dict[str, Any], web_content: str) -> str:
...
"""
    ...
    def generate_md_report(self, cve_id: str) -> str:
        """主函数：生成 Markdown 报告"""
        # 加载数据
        nvd_data = self.load_nvd_data(cve_id)
        if not nvd_data:
            return "# 错误：无法获取 NVD 数据\n\n[NVD 数据缺失]\n"

        web_content = self.load_web_data(cve_id)

        # 构建提示词
        prompt_text = self.create_intel_prompt(nvd_data, web_content)
